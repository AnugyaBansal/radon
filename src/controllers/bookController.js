...
const getBooksList= async function (req, res) {
    let allBooksList= await BookModel.find( ).select({bookName: 1, authorName: 1})
    res.send({msg: allBooksList})
}

const getBooksInYear= async function (req, res) {
    let publishyr = req.body.year
    let allBooksList= await BookModel.find({year: publishyr})
    res.send({msg: allBooksList})
...
const getParticularBooks= async function (req, res) {
    ...
    let allBooksList= await BookModel.find(req.body)
    res.send({msg: allBooksList})
} 

const getXINRBooks= async function (req, res) {
    let allBooksList= await BookModel.find({"prices.indianPrice":{$in:["INR100", "INR200", "INR500"]}})
    res.send({msg: allBooksList})
}

const getRandomBooks= async function (req, res) {
    let allBooksList= await BookModel.find({
        $and:[{stockAvailable: true}, {totalPages:500} ]
})
    res.send({msg: allBooksList})
}
